...
works_count_by_year <- do.call("rbind", lapply(oa_venue$counts_by_year, function(x) data.frame(year=x$year, count=as.integer(x$works_count))))
```

```{r openalex-works-count-plot}
ggplot2::ggplot(works_count_by_year, ggplot2::aes(year, count)) + ggplot2::geom_col() + ggplot2::scale_x_continuous(breaks=rev(works_count_by_year$year)) + ggplot2::ggtitle("PVLDB Works Count (OpenAlex)") + ggplot2::xlab("") + ggplot2::ylab("")
```

## Citations

```{r openalex-citations-count-data}
citation_count_by_year <- do.call("rbind", lapply(oa_venue$counts_by_year, function(x) data.frame(year=x$year, count=as.integer(x$cited_by_count))))
```

```{r openalex-citations-count-plot}
ggplot2::ggplot(citation_count_by_year, ggplot2::aes(year, count)) + ggplot2::geom_col() + ggplot2::scale_x_continuous(breaks=rev(citation_count_by_year$year)) + ggplot2::ggtitle("PVLDB Citations Count (OpenAlex)") + ggplot2::xlab("") + ggplot2::ylab("")
```
